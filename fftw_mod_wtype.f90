...
!!! Finish this one
!!!
  ! This can be optimised to remove one of the forward transforms
  subroutine grad_squared_2d_wtype(this,dk)
    type(transformPair2D), intent(inout) :: this
    real(dl), intent(in) :: dk

    real(C_DOUBLE), dimension(1:this%nx,1:this%ny) :: workspace, grad_x
    integer :: i,ii; real(dl) :: rad2
    complex(C_DOUBLE_COMPLEX) :: norm

    norm = iImag*dk/dble(this%nx)/dble(this%ny)
    workspace = this%realSpace
    call fftw_execute_dft_r2c(this%planf,this%realSpace,this%specSpace)
    do i=1,this%nnx
       this%specSpace(i,:) = norm*(i-1)*this%specSpace(i,:)
    enddo
    call fftw_execute_dft_c2r(this%planb,this%specSpace,this%realSpace)
    grad_x = this%realSpace

    this%realSpace = workspace
    call fftw_execute_dft_r2c(this%planf,this%realSpace,this%specSpace)
    do i=1,this%nny  
       this%specSpace(:,i) = norm*(i-1)*this%specSpace(:,i)
    enddo
    do i=this%nny+1,this%ny
       this%specSpace(:,i) = norm*(i-this%ny-1)*this%specSpace(:,i)
    enddo
    call fftw_execute_dft_c2r(this%planb,this%specSpace,this%realSpace)
    
    this%realSpace = this%realSpace**2 + grad_x**2
  end subroutine grad_squared_2d_wtype

!!!
!!! Finish this one
!!!
  subroutine gradient_energy_2d_wtype(this,dk)
    type(transformPair2D), intent(inout) :: this
    real(dl), intent(in) :: dk
  end subroutine gradient_energy_2d_wtype
